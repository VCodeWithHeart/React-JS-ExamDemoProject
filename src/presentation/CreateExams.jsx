...
const CreateExams = ({ handleInputChange, currentQuestion, handlePrevBtn, handleNextBtn, handleSubjectChange, currentQueIndex, handleNotesChange, questions, handleAnswer, createMode, subjectName, testValidations, editMode, handleEditBtn, reviewing }) => {
    ...
    const { question, sameQuestions, answer, fields, sameOptions, notes } = testValidations
...

                    {
                        <div className='quesDiv' style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                            <TextField name='question'
                                ...
                                error={question?.status || sameQuestions?.status}
                                helperText={question?.status ? question?.msg : sameQuestions?.status ? sameQuestions?.msg : ""}
                            />
                            <div className='optionsDiv' style={{ display: "flex", flexDirection: "column" }}>
                                        ...
                                        currentQuestion?.options?.map((option, optionIndex) => (
                                            <div style={{ display: "flex" }} key={optionIndex}>
                                                <FormControlLabel value={currentQuestion?.options[optionIndex]}
                                                    control={<Radio />}
                                                    disabled={sameOptions?.status}
                                                    checked={!sameOptions?.status && (currentQuestion?.options[optionIndex] && currentQuestion?.answer) &&
                                ...
                                <span style={{ fontSize: "12px", color: "#d32f2f" }}>{answer?.status ? answer?.msg : ""}</span>
                            </div>
                        </div>
                    }


                    <span style={{ fontSize: "12px", color: "#d32f2f" }}>{fields?.status ? fields?.msg : ""}</span>

                    <span style={{ fontSize: "12px", color: "#d32f2f" }}>{sameOptions?.status ? sameOptions?.msg : ""}</span>
                        ...
                        createMode && !editMode ? (
                            <div style={{ display: "flex", flexDirection: "column", gap: "0.3rem" }}>
                                ...
                                <Textarea minRows={2} onChange={(e) => createMode && handleNotesChange(e)} name='notes'
                                    error={notes?.status}
                                />
                                <span style={{ fontSize: "12px", color: "#d32f2f" }}>{notes?.status ? notes?.msg : " "}</span>
                            </div>
                        ) : ""
                    }
